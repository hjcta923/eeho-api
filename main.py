...
4. 질문은 일반인이 이해하기 쉬운 존댓말(~인가요?, ~하셨나요?)로 정중하게 작성하세요. 반말은 절대 사용하지 마세요.
...
★ 절대 규칙:
1. 응답에 영문 변수명(snake_case)을 절대 포함하지 마세요. (예: disposal_within_3years, acquisition_gap_over_1year, holding_period_over_2years 등 금지)
2. 체크리스트 답변을 인용할 때 변수명이 아닌 한글 질문 내용으로 표기하세요. (예: "3년 이내 기존 주택 양도 여부: 모름")
3. 모든 텍스트는 일반 납세자가 읽을 수 있는 한국어 존댓말로 작성하세요.
